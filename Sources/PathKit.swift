...
extension Path {
  ...
  public func normalize() -> Path {
    return Path(try! URL(string: self.path)!.standardized().absoluteString!)
  }
...
extension Path {
  ...
  public var lastComponentWithoutExtension: String {
    return try! URL(string: lastComponent)!.deletingPathExtension().absoluteString!
  }
...
extension Path {
  ...
  public func write(_ data: Data) throws {
    try data.write(to: URL(fileURLWithPath: normalize().path), options: .dataWritingAtomic)
  }
  ...
  public func write(_ string: String, encoding: String.Encoding = String.Encoding.utf8) throws {
    try string.write(toFile: string, atomically: true, encoding: encoding)
  }
}
